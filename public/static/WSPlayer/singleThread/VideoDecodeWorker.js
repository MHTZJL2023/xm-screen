...
function receiveMessage(event) 
{
	...
	switch (message.nType) 
	{
		...
		//开始码流录制
		case 'StartRecord':
			StartRecord(message.nRecordType, message.nFileSize, message.bSegment,message.bNeedResizeRecord);
            break;
		//停止码流录制
		case 'StopRecord':
		case 'CancelRecord':
		...
		case 'SetPlayTokenToDecrypt':
			SetPlayTokenToDecrypt(message.playToken, message.keySalt, message.deviceID, message.bGetPlayInfo);
            break;
		...
		case 'PackageVideoData':
			PackageVideoData(message.width, message.height, message.framerate, message.frameInfo, message.result);
			break;
		//开启鱼眼功能
		case 'StartFisheye':
			StartFisheye();
...
function SetPlayTokenToDecrypt(playToken, keySalt, deviceID, bGetPlayInfo)
{
	...
	var nRet = Module._PLAY_SetPlayTokenToDecrypt(m_nPlayPort, playTokenPtr, keySaltPtr, deviceIDPtr, bGetPlayInfo, stuPlayInfoPtr);
...
function cPlusVisibleDecCallBack(nPort, pBufY, pBufU, pBufV, nSize, pFrameInfo)
{
	...
	{	
		...
		if (0 == stuFrameInfo.bThrowFrame)
		{
			...
			//图像高度
			stuFrameInfo.nHeight = (dataView.getInt32(128, true) > 0) ? dataView.getInt32(128, true) : dataView.getInt32(16, true);
