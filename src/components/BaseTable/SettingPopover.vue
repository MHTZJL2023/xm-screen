...
const onCheckAllChange = (e: any) => {
  ...
  checkedList.value = e.target.checked
    ? defaultOptions.map((item: any) => item.value)
    : [];
...
watch(
  [checkedList, options],
  values => {
    const newColumns: TableColumnType[] = [
      ...props.defaultColumns.filter((item: any) => !item.key),
    ];
    const saveColumns = props.defaultColumns.filter((item: any) => !!item.key);
    // 从option中过滤出排序号的key
    const filterOptionsValue = values[1]
      .filter((item: any) => !!values[0].includes(item.value))
      .map((item: any) => item.value);
    if (values[0]) {
      indeterminate.value =
        !!values[0].length && values[0].length < saveColumns.length;
